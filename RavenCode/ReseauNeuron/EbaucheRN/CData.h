...
public:
  ...
  {
    Init(); //Recupere les donnés de data.txt, 
			//initialise les sets pour le training
    
    //CreateTrainingSetFromData();
  }

  //returns the name of the pattern at val
  string  PatternName(int val); //*****Pas utilisé
  ...
  bool    AddData(vector<double> &data, string NewName); //*****Pas utilisé
  ...
  void CreateTrainingSetFromData(); //*****Pas utilisé
